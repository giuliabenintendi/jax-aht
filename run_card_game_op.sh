...
# ENT=0.01 (default), BETA=0 (no JA reward), 5 seeds, 500k steps.
...
CMD="./run_gpu.sh $GPU marl.run -cn base_config_ja_ippo task=card-game algorithm=ja_ippo/card-game algorithm.TOTAL_TIMESTEPS=5e5 algorithm.JA_BETA_MAX=0.0 algorithm.FEED_OTHER_ATTN=false task.ENV_KWARGS.other_play_position_shuffle=true task.ENV_KWARGS.other_play_recolouring=true"
