...
renderer (semitable/lb-foraging): red food, blue agents.

Level is encoded as color intensity — higher level = more saturated color.

Shapes are distinct so the CNN can distinguish entity types:
  - Empty cell: white with a single grey corner pixel
  - Agent: filled square (inner 5x5)
  - Food: diamond shape (rotated square)
  - Loading agent: orange square
"""
import jax
...
# Colors (RGB uint8)
_GRID_CORNER = jnp.array([190, 190, 190], dtype=jnp.uint8)

# Per-agent base colors (fully saturated = max level)
...
def _make_empty_tile():
    """White tile with a single grey pixel at top-left corner."""
    tile = jnp.full((TILE_PIXELS, TILE_PIXELS, 3), 255, dtype=jnp.uint8)
    tile = tile.at[0, 0, :].set(_GRID_CORNER)
    return tile


def _square_mask():
    """Boolean mask for a filled square (inner 5x5 in a 7x7 tile)."""
    y, x = jnp.meshgrid(
...
def _diamond_mask():
    """Boolean mask for a diamond (rotated square) centered in a 7x7 tile."""
    y, x = jnp.meshgrid(
        jnp.arange(TILE_PIXELS), jnp.arange(TILE_PIXELS), indexing="ij",
    )
    cy, cx = TILE_PIXELS // 2, TILE_PIXELS // 2
    # Manhattan distance <= 2 gives a diamond with radius 2
    return (jnp.abs(y - cy) + jnp.abs(x - cx)) <= 2


_EMPTY_TILE = _make_empty_tile()
_SQUARE = _square_mask()
_DIAMOND = _diamond_mask()
...
def render_lbf_state(state, grid_size, num_agents, num_food):
    ...
    # Draw food items (red diamond, intensity = level)
    def draw_food(img, i):
    ...
    # Draw agents on top (blue square, orange if loading)
