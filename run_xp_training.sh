#!/usr/bin/env bash
# Train JA-IPPO with multiple seeds (single run) for cross-play evaluation.
#
# Usage: ./run_xp_training.sh <gpu_id> <task> <beta> [num_seeds]
#
...
#   ./run_xp_training.sh 5 lbf-image-10food 0.05 6

set -e

GPU="${1:?Usage: ./run_xp_training.sh <gpu_id> <task> <beta> [num_seeds]}"
TASK="${2:?Usage: ./run_xp_training.sh <gpu_id> <task> <beta> [num_seeds]}"
BETA="${3:?Usage: ./run_xp_training.sh <gpu_id> <task> <beta> [num_seeds]}"
SEEDS="${4:-4}"
LABEL=xp_seeds
