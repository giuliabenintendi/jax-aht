...
# position shuffle, 5 seeds each, 500k steps. Expectation:
...
CMD="./run_gpu.sh $GPU marl.run -cn base_config_ja_ippo task=card-game-op-test algorithm=ja_ippo/card-game-op-test"

nohup bash -c "
echo \"[\$(date +%H:%M)] baseline: no OP\"
$CMD task.ENV_KWARGS.op_position_shuffle=false label=op_test_noop_s5

echo \"[\$(date +%H:%M)] OP: position shuffle enabled\"
$CMD task.ENV_KWARGS.op_position_shuffle=true  label=op_test_op_s5

echo \"[\$(date +%H:%M)] Done\"
" > logs/card_game_op_test.log 2>&1 &

echo "Launched OP-test diagnostic (2 runs x 5 seeds) on GPU $GPU"
