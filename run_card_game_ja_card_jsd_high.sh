#!/usr/bin/env bash
# JA_CARD_JSD_COEF beta sweep with everything else stripped: CONC/FOLLOW/ALIGN
# zeroed AND JA_CARD_PARTNER_FEED disabled, so the policy receives no
# OP-translated partner-attention input and JSD is the only JA-related signal.
# This isolates the question "can the JSD reward alone train joint attention?".
#
# 3 vmapped seeds per beta value. Other settings match
# run_card_game_ja_card_sweep.sh so curves are comparable apart from the
...
echo "  CONC=ALIGN=FOLLOW=0, JA_CARD_PARTNER_FEED=false; only JA_CARD_JSD_COEF active"
