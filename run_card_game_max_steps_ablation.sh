...
# 12M total / 6M warmup (= the budget used for prior 8-step runs that we
# want to extend). Warmup stays at 50% of total in every cell.
#
# resulting per-cell budgets:
#   max_steps=2  -> total=3M   warmup=1.5M   (~1465 updates)
#   max_steps=4  -> total=6M   warmup=3M     (~1465 updates)
#   max_steps=12 -> total=18M  warmup=9M     (~1465 updates)
#   max_steps=16 -> total=24M  warmup=12M    (~1465 updates)
#
...
# Anchor: max_steps=8 -> 12e6 total / 6e6 warmup. Per-step budgets below
# preserve those ratios.
STEPS_PER_BLOCK=1500000         # 12e6 / 8
WARMUP_PER_BLOCK=750000         # 6e6 / 8
