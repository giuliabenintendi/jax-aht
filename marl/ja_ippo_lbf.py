...
class TransitionJA(NamedTuple):
    ...
    partner_prev_argmax: jnp.ndarray   # (num_actors,) int32 — aux target
...
def make_train(config, env):
    ...
    def init(rng):
        ...
        # Partner's previous-step per-fruit attention/argmax. Used as supervision
        # target for the aux loss and as the target fruit for r_shape on the
        # NEXT rollout step. Reset to invalid on episode boundaries.
        ...
        runner_state = (
            ...
            init_partner_argmax, init_partner_valid,
        )
        ...
        def step_fn(runner_state, update_steps):
            def _env_step(runner_state, unused):
                (train_state, env_state, last_obs, last_done, hstate, rng,
                 prev_partner_argmax, prev_partner_valid) = runner_state
                ...
                # Partner's argmax = swap own argmax across agent halves
                ...
                runner_state = (
                    ...
                    new_partner_argmax, new_partner_valid,
                )
            ...
            runner_state = (
                train_state, env_state, last_obs, last_done, hstate, rng,
                prev_partner_argmax, prev_partner_valid,
            )
