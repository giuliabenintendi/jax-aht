...
signal, not through gradients. Cross-agent coordination comes from:
...
def make_train_loop(config, env):
    ...
    def init_state(rng, policy):
        """Initialize runner state with separate params per agent."""
        rng, init_rng_0, init_rng_1 = jax.random.split(rng, 3)
        _, init_params_0 = agent_init_fn(config, env, init_rng_0)
        _, init_params_1 = agent_init_fn(config, env, init_rng_1)
        ...
        train_state_0 = TrainState.create(
            apply_fn=policy.network.apply, params=init_params_0, tx=tx,
        )
        train_state_1 = TrainState.create(
            apply_fn=policy.network.apply, params=init_params_1, tx=tx,
        )
        ...
        if feed_other_attn:
            ...
            runner_state = (train_state_0, train_state_1, env_state, obsv, init_done,
                            init_hstate_0, init_hstate_1, _rng, init_other_attn)
        else:
            runner_state = (train_state_0, train_state_1, env_state, obsv, init_done,
    ...
    def make_step_fn(policy):
        ...
        def _single_step(runner_state, update_steps, rew_norm_state):
            ...
            def _env_step(runner_state, unused):
                if feed_other_attn:
                    (train_state_0, train_state_1, env_state, last_obs, last_done,
                     hstate_0, hstate_1, rng, prev_other_attn) = runner_state
                else:
                    (train_state_0, train_state_1, env_state, last_obs, last_done,
                ...
                attn_map = jnp.concatenate([attn_map_0, attn_map_1], axis=1)
                ...
                attn_0_ja = attn_map[:, :num_envs, ...]
                attn_1_ja = attn_map[:, num_envs:, ...]
                r_ja = -jsd_divergence(attn_0_ja.squeeze(0), attn_1_ja.squeeze(0))
                ...
                if feed_other_attn:
                    ...
                    runner_state = (train_state_0, train_state_1, new_env_state, new_obs, new_done,
                                    new_hstate_0, new_hstate_1, rng, new_other_attn)
                else:
                    runner_state = (train_state_0, train_state_1, new_env_state, new_obs, new_done,
            ...
            # Bootstrap values separately per agent
            last_obs_batch = batchify(last_obs, env.agents, num_actors)
            last_done_batch = batchify(last_done, env.agents, num_actors)
            if feed_other_attn:
                last_obs_batch = _augment_obs_with_attn(last_obs_batch, prev_other_attn)
            ...
            _, last_val_0, _, _, _ = policy.get_action_value_policy(
                ...
                hstate=hstate_0, rng=jax.random.PRNGKey(0),
            )
            _, last_val_1, _, _, _ = policy.get_action_value_policy(
                ...
                avail_actions=last_avail_batch[num_envs:].reshape(1, num_envs, -1),
                hstate=hstate_1, rng=jax.random.PRNGKey(0),
            )
            ...
            (total_loss_0, (vl0, pl0, ent0)), gn0 = loss_info_0
            (total_loss_1, (vl1, pl1, ent1)), gn1 = loss_info_1
            ...
            metric["loss_value"] = (vl0[0].mean() + vl1[0].mean()) / 2
            metric["loss_policy"] = (pl0[0].mean() + pl1[0].mean()) / 2
            metric["entropy"] = (ent0.mean() + ent1.mean()) / 2
            metric["grad_norm"] = (gn0.mean() + gn1.mean()) / 2
            ...
            if feed_other_attn:
                runner_state = (train_state_0, train_state_1, env_state, last_obs, last_done,
                                hstate_0, hstate_1, rng, prev_other_attn)
            else:
                runner_state = (train_state_0, train_state_1, env_state, last_obs, last_done,
...
def run_ja_ippo_no_share(config, logger):
    ...
    print(f"[ja_ippo_no_share] Initializing policy and {num_seeds} seeds...")
    ...
    for seed_idx in range(num_seeds):
        ...
        print(f"[ja_ippo_no_share] Seed {seed_idx}/{num_seeds}: training {num_updates} steps...")
        for chunk_end in chunk_boundaries:
            ...
            # Checkpoint agent 0 params after each chunk
            if len(seed_ckpts) < num_ckpts:
                seed_ckpts.append(jax.tree.map(jnp.copy, runner_state[0].params))

            print(f"[ja_ippo_no_share]   step {steps_done}/{num_updates}")

        # Save agent 0 params as "final_params" for XP eval compatibility
